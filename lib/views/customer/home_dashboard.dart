...
import '../booking/rating_sheet.dart';
...
class _HomeDashboardState extends State<HomeDashboard>
  ...
  Future<void> _makeCall(String? phoneNumber) async {
    if (phoneNumber == null || phoneNumber.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text("Technician's number not available yet.")),
      );
      return;
    }
      ...
      body: FadeTransition(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            // ── Completed unrated jobs — rate now banner ───────────────
            SliverToBoxAdapter(
                child: _buildCompletedUnratedBanner()),
  ...
  // ── Completed unrated jobs banner ──────────────────────────────────────────
  // Queries jobs where status=completed AND rated=false for current user
  // Each card opens the rating sheet directly (Option A)
  Widget _buildCompletedUnratedBanner() {
    final uid = FirebaseAuth.instance.currentUser?.uid;
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          ...
          .snapshots(),
      builder: (context, snapshot) {
        // Hide entirely when no unrated completed jobs
        if (!snapshot.hasData ||
            snapshot.data!.docs.isEmpty) {
          return const SizedBox.shrink();
        }

        final unratedJobs = snapshot.data!.docs;

        return Column(
          children: unratedJobs.map((doc) {
            final data =
            ...
            return Container(
              ...
              child: Material(
                ...
                child: InkWell(
                  ...
                  onTap: () async {
                    // Option A — open rating sheet directly
  ...
  // ── Active jobs stream ─────────────────────────────────────────────────────
  // Only shows pending + accepted — completed jobs handled by banner above
  ...
  // ── Workers stream ─────────────────────────────────────────────────────────
  // Only shows approved and non-suspended workers
